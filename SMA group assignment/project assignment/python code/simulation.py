...
class Simulation:
    ...
    def setWeekSchedule(self) -> None:
        ...
        for d in range(self.D):
            time = 8
            session_start = 8        # tracks start of current session (8=morning, 13=afternoon)
            elective_in_session = 0  # elective slot counter within current session (Rule 2)
            elective_block_count = 0 # elective slot counter within current session (Rule 3)
            block_start_time = 8     # start time of current 2-slot block (Rule 3)

            for s in range(self.S):
                # detect session change (morning -> afternoon)
                if time == 13 and session_start == 8:
                    session_start = 13
                ...
                if (self.weekSchedule[d][s].slotType != 1):
                    ...
                    if (self.rule == 1):
                        # Plain FCFS: appointment time = slot start time
                        self.weekSchedule[d][s].appTime = time
                    elif (self.rule == 2):
                        # Bailey-Welch rule:
                        # first 2 elective slots of session -> session start time
                        # remaining slots -> slot start - 1 slot length, capped at session start
                        if elective_in_session < 2:
                            self.weekSchedule[d][s].appTime = session_start
                        else:
                            self.weekSchedule[d][s].appTime = max(session_start, time - self.slotLength)
                        elective_in_session += 1
                    elif (self.rule == 3):
                        # Blocking rule (B=2):
                        # elective slots grouped into blocks of 2
                        # both slots in a block get appointment time = start of first slot in block
                        ...
                        # Benchmarking rule:
                        # appointment time = slot start - k_a * sigma_e, capped at session start
                        # k_a = 0.5, sigma_e = stdevElectiveDuration (in min) -> convert to hours
                        k_a = 0.5
                        offset = k_a * self.stdevElectiveDuration / 60
