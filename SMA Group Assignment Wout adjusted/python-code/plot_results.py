...
STRATEGY_COLORS = {1: PALETTE["blue"], 2: PALETTE["orange"], 3: PALETTE["green"]}
RULE_COLORS     = {1: PALETTE["blue"], 2: PALETTE["orange"], 3: PALETTE["green"], 4: PALETTE["red"]}
...
def plot_figure_A2_interactions(df: pd.DataFrame, plots_dir: Path) -> None:
    ...
    for ax, strategy in zip(axes, strategies):
        ...
        for rule in sorted(sub["rule"].unique()):
            ...
            line = ax.plot(r["n_urgent"], r["W"], marker="o", linewidth=1.8,
                           color=RULE_COLORS.get(rule), label=label_rule(rule))[0]
...
def load_per_rep_combined(path: Path) -> dict[ConfigKey, pd.Series]:
    """Load a combined per-replication CSV (strategy, n_urgent, rule, rep, objective, ...)
    and return the same dict format as load_replication_objectives."""
    if not path.exists():
        return {}
    ...
    obj_col = next((c for c in ["objective", "W", "w"] if c in df.columns), None)
    rep_col = next((c for c in ["rep", "replication", "r"] if c in df.columns), None)
    if obj_col is None:
        print(f"Skipping per-rep combined: no objective column in {path}")
        return {}

    series: dict[ConfigKey, pd.Series] = {}
    for (s, n, r), grp in df.groupby(["strategy", "n_urgent", "rule"]):
        key = ConfigKey(int(s), int(n), int(r))
        idx = grp[rep_col].astype(int) if rep_col else pd.RangeIndex(len(grp))
        series[key] = pd.Series(grp[obj_col].to_numpy(dtype=float), index=idx, name=key)

    print(f"Loaded per-replication data for {len(series)} configurations from {path}")
    return series


def plot_replication_convergence(
    per_rep_series: dict[ConfigKey, pd.Series],
    config: ConfigKey,
    plots_dir: Path,
) -> None:
    """Running mean ± 95% CI as R grows, for one configuration. Justifies sample size."""
    if config not in per_rep_series:
        print(f"Skipping convergence plot: {config} not in per-rep data")
        return

    vals = per_rep_series[config].dropna().to_numpy()
    R = len(vals)
    if R < 3:
        print("Skipping convergence plot: fewer than 3 replications")
        return

    from scipy import stats as scipy_stats

    ns = np.arange(1, R + 1)
    running_mean = np.cumsum(vals) / ns
    running_std = np.array([vals[:i + 1].std(ddof=1) if i >= 1 else 0.0 for i in range(R)])
    tcrit = np.array([float(scipy_stats.t.ppf(0.975, max(n - 1, 1))) for n in ns])
    half_w = tcrit * running_std / np.sqrt(ns)
    half_w[0] = 0.0
    rel_hw = np.where(np.abs(running_mean) > 1e-12, half_w / np.abs(running_mean) * 100, np.nan)

    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(9, 8))

    ax1.plot(ns, running_mean, linewidth=2, label="Running mean W")
    ax1.fill_between(ns, running_mean - half_w, running_mean + half_w, alpha=0.2, label="95% CI")
    ax1.axhline(running_mean[-1], color="gray", linestyle="--", linewidth=1, alpha=0.7)
    ax1.set_xlabel("Number of replications")
    ax1.set_ylabel("Running mean W")
    ax1.set_title(
        f"Convergence of mean W — S{config.strategy}, N={config.n_urgent}, {label_rule(config.rule)}",
        fontweight="bold",
    )
    ax1.legend()
    ax1.grid(alpha=0.3)

    ax2.plot(ns[1:], rel_hw[1:], linewidth=2, color="tab:orange")
    ax2.axhline(5.0, color="red", linestyle="--", linewidth=1.5, label="5% precision target")
    ax2.set_xlabel("Number of replications")
    ax2.set_ylabel("Relative CI half-width (%)")
    ax2.set_title("Precision convergence: relative CI half-width")
    ax2.legend()
    ax2.grid(alpha=0.3)

    save_figure(fig, plots_dir, "figB1_replication_convergence.png")
...
def main() -> None:
    ...
    # Also load the combined per-replication CSV written by write_experiment.
    per_rep_path = experiment_file.parent / (experiment_file.stem + "_per_rep.csv")
    combined_series = load_per_rep_combined(per_rep_path)
    for k, v in combined_series.items():
        if k not in replication_series:
            replication_series[k] = v
    ...
    # Convergence plot for baseline (S1, N=14, R=FCFS) to justify sample size.
    baseline_key = ConfigKey(strategy=1, n_urgent=14, rule=1)
    plot_replication_convergence(replication_series, baseline_key, plots_dir)
