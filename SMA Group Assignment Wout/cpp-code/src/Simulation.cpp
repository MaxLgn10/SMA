    ...
    for (s = 0; s < 32; s++) {
        for (d = 0; d < D; d++) {
            if (!(inputFile >> elementInt)) {
                cerr << "ERROR: input file must contain 32 rows and 6 columns of integers." << endl;
                exit(1);
            }
    ...
    for (d = 0; d < D; d++) {
        ...
        for (s = 0; s < S; s++) {
            ...
            if (weekSchedule[d][s].slotType != 1) {
                ...
                // Blocking with block size 2: two elective slots get the same appointment time.
                ...
                // k_a=0.5 and sigma_e is the elective duration standard deviation in minutes.
                const double ka = 0.5;
...
void simulation::schedulePatients() {
    patients.sort([](const Patient &p1, const Patient &p2) {
        ...
        if (p1.patientType == 2 && p2.patientType != 2) return true;
        if (p2.patientType == 2 && p1.patientType != 2) return false;
        return p1.nr < p2.nr;
    });
    ...
    for (patient = patients.begin(); patient != patients.end(); ++patient) {
        ...
        if (patient->callWeek == week[i] && patient->callDay == day[i] &&
            ...
            if (patient->patientType == 2 || patient->callTime < weekSchedule[day[i]][lastSlotOfType].appTime) {
...
void simulation::runOneSimulation() {
    ...
    for (patient = patients.begin(); patient != patients.end(); ++patient) {
        ...
        bool newDay = (patient->scanWeek != prevWeek || patient->scanDay != prevDay);
        if (newDay) {
            close_previous_day();
            prevIsNoShow = false;
        }
    ...
    if (totalElectiveScanned > 0) avgElectiveScanWT /= totalElectiveScanned;
    if (totalUrgentScanned > 0) avgUrgentScanWT /= totalUrgentScanned;
    avgOT = totalOT / (D * W);
}
...
void simulation::runSimulations() {
    ...
    for (r = 0; r < R; r++) {
        ...
        printf("%d,%.8f,%.8f,%.8f,%.8f,%.8f\n", r, avgElectiveAppWT, avgElectiveScanWT,
               avgUrgentScanWT, avgOT, objective);
    }
