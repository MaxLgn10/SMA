...
void write_single_config(const string &inputFile, int rule, int W, int R,
                         int warmupWeeks, bool urgentTwoBlocks,
  ...
  cout << fixed << setprecision(6) << "Mean objective = " << obj.mean << " ["
       << obj.lo << ", " << obj.hi << "]\n"
       << "Mean elective appointment WT = " << el.mean << " h\n"
       << "Mean urgent scan WT = " << ur.mean << " h\n";
}

void write_warmup(const string &inputFile, int rule, int W, int R,
  ...
  vector<double> sumElApp(W, 0.0), sumElScan(W, 0.0), sumUrScan(W, 0.0),
      sumOT(W, 0.0);
  ...
  out << "week,avg_el_app_wt_hrs,avg_el_scan_wt_hrs,avg_ur_scan_wt_hrs,avg_ot_"
         "hrs,avg_objective\n";
  for (int w = 0; w < W; w++) {
...
void write_experiment(int W, int R, int warmupWeeks, bool urgentTwoBlocks,
                      const string &inputDir, const string &outFile) {
  string outDir = filesystem::path(outFile).parent_path().string();
  ofstream out(outFile);
  if (!out.is_open()) {
    cerr << "ERROR: cannot write " << outFile << endl;
    exit(1);
  ...
  out << "strategy,n_urgent,rule,mean_objective,ci_lo_objective,ci_hi_"
         "objective,"
  ...
  for (int strategy = 1; strategy <= 3; strategy++) {
    for (int nUrgent = 10; nUrgent <= 20; nUrgent++) {
      ...
      filename << inputDir << "/input-S" << strategy << "-" << nUrgent
               << ".txt";
      ...
      for (int rule = 1; rule <= 4; rule++) {
        ...
        ostringstream repFile;
        repFile << outDir << "/replication_analysis_S" << strategy << "N"
                << nUrgent << "R" << rule << ".csv";
        ...
        ofstream repOut(repFile.str());
        if (repOut.is_open()) {
          repOut << "replication,el_app_wt,ur_scan_wt,objective\n";
          for (int r = 0; r < R; r++) {
            repOut << r << "," << setprecision(10) << elApps[r] << ","
                   << urScans[r] << "," << objectives[r] << "\n";
          }
          repOut.close();
        } else {
          cerr << "ERROR: cannot write " << repFile.str() << endl;
        }
        ...
        cerr << "[" << done << "/" << total << "] S" << strategy
             << " N=" << nUrgent << " rule=" << rule << " objective=" << fixed
             << setprecision(5) << obj.mean << endl;

        out << strategy << "," << nUrgent << "," << rule << ","
            << setprecision(10) << obj.mean << "," << obj.lo << "," << obj.hi
            << "," << el.mean << "," << el.lo << "," << el.hi << "," << ur.mean
            << "," << ur.lo << "," << ur.hi << "," << elScan.mean << ","
            << ot.mean << "\n";
      }
    }
  }
...
void print_usage(const char *program) {
  cout << "Usage:\n"
       << "  " << program
       << " single [inputFile] [rule] [W] [R] [warmup] [urgentTwoBlocks=0|1] "
          "[outFile]\n"
       << "  " << program
       << " warmup [inputFile] [rule] [W] [R] [urgentTwoBlocks=0|1] [outFile]\n"
       << "  " << program
       << " experiment [W] [R] [warmup] [urgentTwoBlocks=0|1] [inputDir] "
          "[outFile]\n";
}
...
int main(int argc, const char *argv[]) {
  ...
  if (argc == 1) {
    ...
    write_single_config("../input-S1-14.txt", 1, 100, 100, 0, false,
                        "../results/single_run_results.csv");
    print_usage(argv[0]);
    return 0;
  }
  ...
  if (mode == "single") {
    ...
    write_single_config(inputFile, rule, W, R, warmup, urgentTwoBlocks,
                        outFile);
  } else if (mode == "warmup") {
    string inputFile = (argc > 2) ? argv[2] : "../input-S1-14.txt";
    int rule = (argc > 3) ? atoi(argv[3]) : 1;
